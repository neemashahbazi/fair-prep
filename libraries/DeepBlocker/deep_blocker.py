...
from libraries.DeepBlocker import blocking_utils
...
class DeepBlocker:
    ...
    def validate_columns(self):
        ...
        self.cols_to_block_without_id = [
            col for col in self.cols_to_block if col != "id"
        ]
        ...
        if not check:
            raise Exception(
                "Not all columns in cols_to_block are present in the left dataset"
            )
        ...
        if not check:
            raise Exception(
                "Not all columns in cols_to_block are present in the right dataset"
            )
    ...
    def preprocess_datasets(self):
        ...
        self.left_df["_merged_text"] = self.left_df[self.cols_to_block_without_id].agg(
            " ".join, axis=1
        )
        self.right_df["_merged_text"] = self.right_df[
            self.cols_to_block_without_id
        ].agg(" ".join, axis=1)
    ...
    def block_datasets(self, left_df, right_df, cols_to_block):
        ...
        all_merged_text = pd.concat(
            [self.left_df["_merged_text"], self.right_df["_merged_text"]],
            ignore_index=True,
        )
        self.tuple_embedding_model.preprocess(all_merged_text)

        print("Obtaining tuple embeddings for left table")
        self.left_tuple_embeddings = self.tuple_embedding_model.get_tuple_embedding(
            self.left_df["_merged_text"]
        )
        print("Obtaining tuple embeddings for right table")
        self.right_tuple_embeddings = self.tuple_embedding_model.get_tuple_embedding(
            self.right_df["_merged_text"]
        )
        ...
        self.candidate_set_df = blocking_utils.topK_neighbors_to_candidate_set(
            topK_neighbors
        )

        return self.candidate_set_df

    def save_intermediate_results(self, left_df, right_df, cols_to_block):
        self.left_df = left_df
        self.right_df = right_df
        self.cols_to_block = cols_to_block
