...
from libraries.DeepBlocker import dl_models
from libraries.DeepBlocker.configurations import *
...
class AverageEmbedding(ABCTupleEmbedding):
        ...
        average_embeddings = np.array(
            [
                np.mean(
                    np.array(
                        [
                            self.word_embedding_model.get_word_vector(token)
    ...
    def get_word_embedding(self, list_of_words):
        return [
            self.word_embedding_model.get_word_vector(word) for word in list_of_words
        ]
...
class SIFEmbedding(ABCTupleEmbedding):
    ...
    def get_tuple_embedding(self, list_of_tuples):
        ...
        for index, _tuple in enumerate(list_of_tuples):
            ...
            tuple_embeddings[index, :] = np.mean(
                np.array(
                    [
                        self.word_embedding_model.get_word_vector(token)
                        * self.token_weight_dict[token]
                        for token in self.tokenizer(_tuple)
                    ]
                ),
                axis=0,
        ...
        if self.remove_pc:
            ...
            sif_embeddings = (
                tuple_embeddings - tuple_embeddings.dot(pc.transpose()) * pc
            )
        else:
...
class AutoEncoderTupleEmbedding(ABCTupleEmbedding):
    def __init__(
        self, hidden_dimensions=(2 * AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE)
    ):
        super().__init__()
    ...
    def preprocess(self, list_of_tuples):
        ...
        trainer = dl_models.AutoEncoderTrainer(
            self.input_dimension, self.hidden_dimensions
        )
    ...
    def get_word_embedding(self, list_of_words):
        embedding_matrix = torch.tensor(
            self.sif_embedding_model.get_tuple_embedding(list_of_words)
        ).float()
        return self.autoencoder_model.get_tuple_embedding(embedding_matrix)
...
def generate_synthetic_training_data(
    list_of_tuples, synth_tuples_per_tuple=5, pos_to_neg_ratio=1, max_perturbation=0.4
):
...
class CTTTupleEmbedding(ABCTupleEmbedding):
    ...
    def preprocess(self, list_of_tuples):
        ...
        trainer = dl_models.CTTModelTrainer(
            self.input_dimension, self.hidden_dimensions
        )
