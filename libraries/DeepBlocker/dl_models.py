...
class NumPy_Triplet_Dataset(Dataset):
    def __init__(self, left_embedding_matrix, right_embedding_matrix, labels):
        ...
        self.labels = labels
        if (len(left_embedding_matrix) != len(right_embedding_matrix)) or (
            len(right_embedding_matrix) != len(labels)
        ):
            raise Exception(
                "The dimensions of left and right embedding matrix and labels do not match"
            )
...
class AutoEncoderTrainer:
    ...
    def train(self, embedding_matrix, num_epochs=NUM_EPOCHS, batch_size=BATCH_SIZE):
        ...
        train_dataloader = DataLoader(
            dataset=NumPy_Dataset(embedding_matrix), batch_size=batch_size, shuffle=True
        )
...
class CTTModelTrainer:
    ...
    def train(
        self,
        left_embedding_matrix,
        right_embedding_matrix,
        labels,
        ...
        dataset = NumPy_Triplet_Dataset(
            left_embedding_matrix, right_embedding_matrix, labels
        )
        train_dataloader = DataLoader(
